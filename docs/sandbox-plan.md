...
E2B's equivalent of `guest-agent` is called `envd`; the naming is worth keeping
distinct to avoid implying shared code.
...
> Reporting a running total double-bills every customer, compounding.
...
4. **Stratos has no rate limiting.** Sandbox creation is expensive and trivially
   abusable.
