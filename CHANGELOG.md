...
- **Boot-from-volume server create with curated storage types** — the client create-server wizard gains a **Storage** step: the root disk becomes a Cinder volume (size + storage class, Nova `block_device_mapping_v2` at microversion 2.67) with optional blank **data volumes** attached at boot. Storage classes are an operator-published catalog (`config.features.volumeTypes[region]`, managed on the provider **Volume types** tab) intersected with live Cinder types — fail-closed when unconfigured. Standalone volume create and retype resolve against the same catalog (create-from-snapshot keeps inheriting the origin's type). Root volumes are billed as VOLUME instead of the flavor's `root_disk_gb`, are deleted with the server via `delete_on_termination` (teardown orders servers first and waits out Nova's async cleanup), and cannot be detached or deleted from the portal while attached; data volumes survive server deletion. Rebuild/rescue of volume-backed servers pin Nova microversions 2.93/2.87. Flavors with local ephemeral/swap disk are hidden from the wizard and rejected for volume-backed create/resize (they stay available for resizing pre-existing image-backed servers).
...
- The **Logs** tab of a healthy Valkey database showed `internal error` and no log lines: the
  ...
  `valkey`. (#170)
- Valkey's `default` account authenticated but answered `NOPERM` to every command: the
  operator renders exactly the ACL rules a user declares, and the chart declared none — a
  `user … on #hash` line with no rules is a valkey user with no keys and no commands. The
  chart (`database-cluster` 0.7.1) now grants `default` the full `~* &* +@all` (the customer's
  own instance — same posture as the postgresql superuser) and customer ACL users
  `~* &* +@all -@admin`, keeping app credentials away from `CONFIG`/`CLUSTER`/`ACL`.
