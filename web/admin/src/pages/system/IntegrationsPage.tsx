...
import { Activity, Plug, Settings2 } from "lucide-react"
...
export default function IntegrationsPage() {
  ...
  for (const e of statsQ.data?.data ?? []) {
    ...
    if (seen) {
      ...
      catalog.push({ name: e.name, categories: [...(e.categories ?? [])], installed: e.installed })
    }
  }
  ...
  const shownCats = tab === "All" ? CATS : CATS.filter((c) => c.label === tab)
  ...
  return (
    <>
      ...
      {/* Installed */}
      <h2 className="mb-3 text-sm font-medium text-muted-foreground">Installed</h2>
      {listQ.isLoading ? (
        <Skeleton className="h-40" />
      ) : listQ.error ? (
        <div className="rounded-lg border bg-muted/40 p-4 text-sm text-muted-foreground">{(listQ.error as Error).message}</div>
      ) : installed.length === 0 ? (
            ...
            <TableBody>
              {installed.map((i) => (
                <TableRow key={i.id}>
                  ...
                  <TableCell>{i.thirdParty ?? "—"}</TableCell>
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {categoriesOf(i.thirdParty).map((c) => (
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-1">
                      <Button variant="ghost" size="sm" onClick={() => health.mutate(i.id)} disabled={health.isPending}>
                        <Activity className="size-4" /> Health
                      </Button>
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => setEditing({ vendor: i.thirdParty ?? i.name ?? "", installed: i })}
                      >
                        Edit
                      </Button>
                      <Button variant="ghost" size="sm" className="text-destructive" onClick={() => setDeleting(i)}>
                        Delete
                      </Button>
                    </div>
                  </TableCell>
      ...
      {statsQ.isLoading ? (
        ...
        <div className="space-y-6">
          {shownCats.map((cat) => {
            ...
            return (
              <section key={cat.key}>
                <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{cat.label}</h3>
                      ...
                      >
                        <CardContent className="p-4">
                          <div className="flex items-start justify-between gap-2">
                            ...
                            {isInstalled ? <Badge>Installed</Badge> : <Badge variant="outline">Available</Badge>}
                          </div>
